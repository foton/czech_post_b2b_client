...
class TryApiCalls
  ...
  def run
    ...
    @transaction_id = nil
    # @transaction_id = 'EE7C4039-0170-4000-E000-1F5D0AA0630B'
    import_parcels = true
    ...
    if import_parcels
      ...
      import_parcels_data
      while ( Time.now.utc < (processing_end_time_utc + 1)) do
  ...
  def existing_parcel_codes
    # If Import of parcels do not work for You yet, You can insert parcels in Web PodaniOnline and then use this hack to check some services
    ["BA0305100278L", "BA0305100264L", "BA0305100255L"]
  end
  ...
  def import_parcels_data
    ...
    self.processing_end_time_utc = (sender_service.result.processing_end_expected_at - (60 *60)).utc # CP returns CET but marked as UTC
    self.transaction_id = sender_service.result.transaction_id
    CzechPostB2bClient.logger.debug("[ParcelsSender] processing_end_time_utc: #{processing_end_time_utc} ; transaction_id: #{transaction_id}")
  end
  ...
  def print_all_template_sheets_for_parcel_codes
    CzechPostB2bClient::PrintingTemplates.all_template_classes.each do |t_klass|
  ...
  def save_as_pdf(pdf_content, options)
    def_str = "-template_id_#{options[:template_id]}-top_#{options[:margin_in_mm][:top]}-left_#{options[:margin_in_mm][:left]}"
    puts('saving:' + def_str)
  ...
  def short_common_data
    {
      ...
      sending_post_office_code: 12000,
